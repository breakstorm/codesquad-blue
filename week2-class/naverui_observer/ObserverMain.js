//(변경)컨트롤러 = Model에 직접 접근하지 못하도록 하는 역할
//controller에 있는 함수 메세지로 변경
//dispatcher에 함수 통째로 등록
DispatcherObj = {
	...
	run: function(o, data){
		debugger;
		return	this.fnlist[o.type].call(null, data);
	}
}
var ControllerObj = {
	init: function(){
...
var ModelObj = {
	...
	debugger;
	ControllerObj.init();
	DispatcherObj.run({"type":"getAllJsonData"});
	myHead.init();
	myNav.init();
	myArticle.init();
	myCompany.init();
